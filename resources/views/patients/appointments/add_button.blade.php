    ...
    <div class="dropdown-menu py-0" aria-labelledby="dropdownMenuButton1">
        ...
        <div class="p-5">
            <div class="mb-5">
                ...
                <input class="form-control form-control-solid" value="{{$filter['date']}}" placeholder="Pick date range" id="patientAppointmentDate"/>
            </div>
            <div class="mb-5">
                <label for="filterBtn" class="form-label">{{__('messages.appointment.payment')}}:</label>
                {{ Form::select('payment_type', \App\Models\Appointment::PAYMENT_TYPE_ALL, $filter['paymentType'],['class' => 'form-control form-control-solid form-select', 'data-control'=>"select2", 'id' => 'patientPaymentStatus']) }}
            </div>
            <div class="mb-5">
                <label for="filterBtn" class="form-label">{{__('messages.doctor.status')}}:</label>
                {{ Form::select('status', $appointmentStatus, $filter['status'],['class' => 'form-control form-control-solid form-select', 'data-control'=>"select2", 'id' => 'patientAppointmentStatus']) }}
            </div>
            <div class="d-flex justify-content-end">
...
<a type="button" class="btn btn-primary" href="{{ route('patients.appointments.create')}}" data-turbo="false">
